...
export interface PipelineAssets {
  ...
  audioFile: string;
  r2BaseUrl: string;
  // Full public URLs – derived in selectAssets, forwarded to templates
  bg1Url: string;
  bg2Url: string;
  musicUrl: string;
  // Human-readable short names for notifications
  names: {
    vid1: string;
    vid2: string;
    aud: string;
...
export class PipelineService {
  ...
  protected async selectAssets(): Promise<PipelineAssets> {
    const { assets } = this.brand;
    const pickRandom = <T>(arr: T[]): T =>
      arr[Math.floor(Math.random() * arr.length)];

    const video1File = pickRandom(assets.videos);
    let video2File = pickRandom(assets.videos);
    while (video2File === video1File) {
      video2File = pickRandom(assets.videos);
    }
    const audioFile = pickRandom(assets.audios);

    const r2BaseUrl = env.R2_PUBLIC_URL.replace(/\/$/, "");
    const bg1Url = `${r2BaseUrl}/${this.brand.r2Folder}/${assets.videoFolder}/${video1File}`;
    const bg2Url = `${r2BaseUrl}/${this.brand.r2Folder}/${assets.videoFolder}/${video2File}`;
    const musicUrl = `${r2BaseUrl}/${this.brand.r2Folder}/${assets.audioFolder}/${audioFile}`;
  ...
  protected async renderAndUpload(
  ...
  protected async sendSuccessNotification(
    _payload: AirtablePayload,
    assets: PipelineAssets,
    postLink: string,
  ): Promise<void> {
    await notifyTelegram(
