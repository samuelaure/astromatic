  ...
  for (let i = 0; i < maxRetries; i++) {
    ...
    if (status_code === "ERROR") {
      throw new DistributionError(
        `Media processing failed: ${response.data.status}`,
        {
          containerId,
...
export async function publishToInstagram(
  ...
  return withRetry(async () => {
    try {
      ...
      logger.info(
        { creationId },
        "Container created. Waiting for processing...",
      );
      ...
      const err = error as {
        response?: {
          data?: { error?: { error_subcode?: number }; status?: string };
        };
        message?: string;
      };
      ...
      if (subcode === 467) {
        errorMessage =
          "Instagram Token Revoked: User logged out or password changed. Please update IG_TOKEN in .env.";
      } else if (subcode === 463) {
